# JUDUL 13 OF 20
## Borcelle Spa – Spa & Body Wellness Center Promotional Suite

**Sub-Topic:** CP-SBM-E-Fashion-Beauty-Jewelry  
**Main Keywords:** borcelle spa, spa & body wellness center, sparelax25, promo banner, loyalty card  
**Target Audience:** Customers, Clients, Local Community, Social Media Followers  
**Style:** Professional Canva Small Business Aesthetic Layout  
**Tone:** Engaging, Friendly, Trustworthy  

**Total Assets:** 9 Aset Promosi Lengkap (IG Story x2, IG Post x2, Loyalty Card, Business Card, Discount Coupon, Menu/Pricelist, Logo)  

---

### ASSET 1 - BRAND LOGO & IDENTITY
**Logo Concept:**
- Logo Name: **Borcelle Spa**
- Tagline: *"Revitalize Your Mind, Body & Soul"*
- Primary Colors: Calming Lavender, Soft Violet & Cream
- Visual Style: Clean, modern, high-readability typography suitable for Canva social media banners and print materials.

---

### ASSET 2 - IG STORY 1: "WE'RE OPEN TODAY" (OPERATIONAL ANNOUNCEMENT)
**Format:** Vertical Story (1080x1920) | **Style:** Fast, Fresh, Casual Tone
**Text Content:**
- Header Tag: WE'RE OPEN TODAY! 📍
- Highlight: Ready to serve you the best Traditional Aromatherapy Massage & Facial Spa.
- Operating Hours: 09.00 AM — 08.00 PM (Open Daily)
- Location Pin: 123 Anywhere St., Any City, ST 12345
- Interactive Sticker: Tap link below to get instant directions / WhatsApp chat!

---

### ASSET 3 - IG STORY 2: FLASH SALE & SPECIAL PROMO
**Format:** Vertical Story (1080x1920) | **Style:** High Urgency, Eye-Catching Deal
**Text Content:**
- Big Headline: SPECIAL PROMO — 25% OFF
- Subtext: Exclusive offer for our signature Traditional Aromatherapy Massage & Facial Spa!
- Coupon Code: Use code **SPARELAX25** at checkout.
- Expiry Notice: ⏰ Valid until September 02, 2035 (Limited Slots)
- Call to Action: Swipe Up / Tap link in bio to claim your promo code now!

---

### ASSET 4 - IG POST 1: SERVICE FEATURE SPOTLIGHT BANNER
**Format:** Square Feed (1080x1080) | **Style:** Aesthetic Service Showcase Feed Ad
**Text Content:**
- Logo Header: **Borcelle Spa**
- Big Headline: PURE AROMATHERAPY RELAXATION
- Subtitle: REJUVENATE YOUR BODY AND MIND
- Feature Badge: ORGANIC ESSENTIAL OILS & LICENSED THERAPISTS
- Visual Element: Relaxing spa candle and essential oil bottle artwork on warm soothing green layout
- Caption Copy: "Melt away stress and muscle tension with our 90-minute signature aromatherapy massage using organic botanical oils."
- CTA Button: Reserve Spa Session / Book Today
- Footer Contact: www.reallygreatsite.com | +123-456-7890


---

### ASSET 5 - IG POST 2: FEATURED SELECTION & ITEM SHOWCASE
**Format:** Square Feed (1080x1080) | **Style:** Detailed Product/Service Showcase Card
**Text Content:**
- Title: OUR FEATURED SELECTIONS – Borcelle Spa
- Highlighted Menu/Service Showcase:
  - **Deep Hydrating Botanical Facial** ($38.00/60min) – Deep cleansing, steam extraction, and hydrating mask.
- Subtext: Crafted with precision, passion, and premium quality.
- CTA: Tap link in bio to view full menu and price list!

---

### ASSET 6 - PROMOTION LOYALTY CARD
**Layout:** Standard Wallet Stamp Card (Front & Back)
**Front Side:**
- Title: LOYALTY CARD – Borcelle Spa
- Reward Rule: Book 8 spa sessions to get 1 Free Organic Facial Treatment
- Stamp Grid: 8 Stamp Slots (Slot 8 = FREE REWARD)
**Back Side:**
- Logo: **Borcelle Spa**
- Tagline: *"Revitalize Your Mind, Body & Soul"*
- Terms: Non-transferable. Present card upon purchase to collect stamps.

---

### ASSET 7 - BRAND BUSINESS CARD
**Layout:** Standard Business Card Size (Front & Back - Canva Professional Layout)  
**Style & Aesthetics:** Serene Zen Spa Aesthetic with Soft Olive/Sage Front & Off-White Paper Back  

**Front Side (Brand Cover Side):**
- **Background:** Soft Muted Sage Green (`#8A9A86`)
- **Center Logo & Illustration:** Off-white line art vector logo of a blooming lotus flower & water droplet ripple
- **Brand Name:** **BORCELLE SPA** (Calm organic serif display typography, centered)
- **Tagline:** *"Revitalize Your Mind, Body & Soul"* (Centered subtitle in delicate cream text, in quotes)

**Back Side (Contact Details & Visual Art Side):**
- **Background:** Off-White Linen Canvas (`#F7F6F0`)
- **Left Column (Personal & Contact Details):**
  - **Contact Person:** **MAYA LIN** (Serenity title typography)
  - **Role / Job Title:** `Spa Manager & Wellness Consultant` (Clean secondary subheader)
  - **Contact Details (Sage Green Pill Badges):**
    - ✉️ Email Badge: `hello@borcellespa.com` (Solid sage green capsule, white text)
    - 📞 Phone Badge: `+123-456-7890` (Solid sage green capsule, white text)
    - 🌐 Website Badge: `borcellespa.com` (Solid sage green capsule, white text)
  - **Address Line:** `📍 123 Anywhere St., Any City` (Location pin icon + address text line)
- **Right Column (Topic Visual Artwork):**
  - **Visual Illustration:** Detailed sage green vector line art illustration of a blooming lotus flower, essential oil bottle & bamboo water spout with sparkle stars (`✦` / `✨`)

---

### ASSET 8 - DISCOUNT VOUCHER COUPON
**Layout:** Horizontal Promotional Voucher Card Set (3 High-Impact Design Variations - Canva & Print Ready)  
**Style & Aesthetics:** Tranquil Zen Spa Theme with Deep Forest Emerald & Gold Background & Wellness Photography  

#### VOUCHER VARIATION 1: SPECIAL PERCENTAGE DISCOUNT (`30% OFF`)
- **Background & Palette:** Deep Forest Emerald & Gold Gradient (`#1B4332` -> `#081C15`)
- **Top Left Header:** Brand Logo (**Borcelle Spa**)
- **Main Headline:** **DISCOUNT COUPON** (Bold condensed white display typography)
- **Giant Offer Highlight:** **30% OFF** (Giant centered white typography)
- **Promo Code Pill Badge:** `CODE : RELAX30` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution product photography of a serene blooming lotus flower floating on water with essential oil bottle & water droplet ripple

#### VOUCHER VARIATION 2: B1G1 SPECIAL DEAL (`BUY 1 GET 1`)
- **Background & Palette:** Rich Muted Emerald Solid (`#2D6A4F`)
- **Top Left Header:** Brand Logo (**Borcelle Spa**)
- **Main Headline:** **BUY 1 GET 1** (Bold white uppercase display typography)
- **Giant Offer Highlight:** **BUY 1 GET 1** (Giant high-contrast white text highlight)
- **Promo Code Pill Badge:** `CODE : B1G1` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution photo of smooth hot massage basalt stones & bamboo stalk with steam vapor mist

#### VOUCHER VARIATION 3: COUPLES DELUXE SPA DAY BUNDLE (`40% OFF`)
- **Background & Palette:** Deep Dark Botanical Emerald Gradient (`#143601` -> `#061D00`)
- **Top Left Header:** Brand Logo (**Borcelle Spa**)
- **Main Headline:** **COUPLES SPA DAY** (Bold white header typography)
- **Giant Offer Highlight:** **40% OFF** (Giant bold white text highlight)
- **Promo Code Pill Badge:** `CODE : ZEN40` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution photo showcase of luxury candlelit spa room suite with flower petal bath & warm glowing aura

---

### ASSET 9 - MENU & PRICELIST
**Layout:** A4 / Printable Price List Sheet
**Header:** **Borcelle Spa – MENU & PRICELIST**
**Products / Services & Specific Pricing:**
- **Signature Aromatherapy Massage (90 min)** – $48.00
- **Deep Hydrating Botanical Facial (60 min)** – $38.00
- **Himalayan Salt Body Scrub & Bath** – $42.00
- **Express Head & Shoulder Massage (30 min)** – $20.00
**Footer Note:** All prices include applicable taxes. Contact us for custom bookings!