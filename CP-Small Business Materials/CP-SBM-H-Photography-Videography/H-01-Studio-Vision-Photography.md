# JUDUL 19 OF 20
## Studio Vision – Commercial & Wedding Photography Promotional Suite

**Sub-Topic:** CP-SBM-H-Photography-Videography  
**Main Keywords:** studio vision, commercial & wedding photography, photo40, promo banner, loyalty card  
**Target Audience:** Customers, Clients, Local Community, Social Media Followers  
**Style:** Professional Canva Small Business Aesthetic Layout  
**Tone:** Engaging, Friendly, Trustworthy  

**Total Assets:** 10 Aset Promosi Lengkap (IG Story x3, IG Post x2, Loyalty Card, Business Card, Discount Coupon, Menu/Pricelist, Logo)  

---

### ASSET 1 - BRAND LOGO & IDENTITY
**Logo Concept:**
- Logo Name: **Studio Vision**
- Tagline: *"Capturing Timeless Moments Forever"*
- Primary Colors: Modern Dark Charcoal, Gold & White
- Visual Style: Clean, modern, high-readability typography suitable for Canva social media banners and print materials.

---

### ASSET 2 - IG STORY 1: "WE'RE OPEN TODAY" (OPERATIONAL ANNOUNCEMENT)
**Format:** Vertical Story (1080x1920) | **Style:** Fast, Fresh, Casual Tone
**Text Content:**
- Header Tag: WE'RE OPEN TODAY! 📍
- Highlight: Ready to serve you the best Pre-Wedding Shoot & Wedding Day Photography.
- Operating Hours: 09.00 AM — 07.00 PM (Tuesday - Sunday)
- Location Pin: 123 Anywhere St., Any City, ST 12345
- Interactive Sticker: Tap link below to get instant directions / WhatsApp chat!

---

### ASSET 3 - IG STORY 2: FLASH SALE & SPECIAL PROMO
**Format:** Vertical Story (1080x1920) | **Style:** High Urgency, Eye-Catching Deal
**Text Content:**
- Big Headline: SPECIAL PROMO — FLAT $40 OFF
- Subtext: Exclusive offer for our signature Pre-Wedding Shoot & Wedding Day Photography!
- Coupon Code: Use code **PHOTO40** at checkout.
- Expiry Notice: ⏰ Valid until September 02, 2035 (Limited Slots)
- Call to Action: Swipe Up / Tap link in bio to claim your promo code now!

---

### ASSET 4 - IG STORY 3: "THANKS FOR VISITING OUR STUDIO" (CUSTOMER APPRECIATION)
**Format:** Vertical Story (1080x1920) | **Style:** Soft Warm Aesthetic, Rounded Photo Frame & Heart Badge
**Text Content:**
- Top Headline: THANKS FOR VISITING OUR STUDIO
- Visual Element: Rounded photo frame of happy customer / staff with heart sticker badge
- Brand Name: **Studio Vision**
- Closing Message: "We can't wait to capture your next unforgettable moment. See you soon!"

---

### ASSET 5 - IG POST 1: SERVICE FEATURE SPOTLIGHT BANNER
**Format:** Square Feed (1080x1080) | **Style:** Aesthetic Service Showcase Feed Ad
**Text Content:**
- Logo Header: **Studio Vision**
- Big Headline: CAPTURING PRECIOUS MOMENTS
- Subtitle: TIMELESS VISUAL STORYTELLING
- Feature Badge: HIGH-DEFINITION 4K & PROFESSIONAL LIGHTING
- Visual Element: Camera aperture lens artwork with elegant photo frame on dark studio aesthetic layout
- Caption Copy: "Preserve your most cherished memories with breathtaking photography, artistic framing, and professional color grading."
- CTA Button: Book Shoot / Reserve Studio
- Footer Contact: www.reallygreatsite.com | +123-456-7890


---

### ASSET 6 - IG POST 2: FEATURED SELECTION & ITEM SHOWCASE
**Format:** Square Feed (1080x1080) | **Style:** Detailed Product/Service Showcase Card
**Text Content:**
- Title: OUR FEATURED SELECTIONS – Studio Vision
- Highlighted Menu/Service Showcase:
  - **Outdoor Pre-Wedding Concept Shoot** ($260.00) – 4 hours shoot, 2 locations, 20 color-graded retouched photos.
- Subtext: Crafted with precision, passion, and premium quality.
- CTA: Tap link in bio to view full menu and price list!

---

### ASSET 7 - PROMOTION LOYALTY CARD
**Layout:** Standard Wallet Stamp Card (Front & Back)
**Front Side:**
- Title: LOYALTY CARD – Studio Vision
- Reward Rule: Book Wedding Photo Session to get 1 Free 20x30 Canvas Frame
- Stamp Grid: 5 Stamp Slots (Slot 5 = FREE REWARD)
**Back Side:**
- Logo: **Studio Vision**
- Tagline: *"Capturing Timeless Moments Forever"*
- Terms: Non-transferable. Present card upon purchase to collect stamps.

---

### ASSET 8 - BRAND BUSINESS CARD
**Layout:** Standard Business Card Size (Front & Back - Canva Professional Layout)  
**Style & Aesthetics:** Sleek Minimalist Photography Theme with Matte Black Front & Fine Grid Back  

**Front Side (Brand Cover Side):**
- **Background:** Matte Charcoal Black (`#121212`)
- **Center Logo & Illustration:** White minimalist line art vector logo of a camera lens aperture iris with flash glare rays
- **Brand Name:** **STUDIO VISION** (Bold high-contrast geometric sans typography, centered)
- **Tagline:** *"Capturing Timeless Moments Forever"* (Centered subtitle in crisp white text, in quotes)

**Back Side (Contact Details & Visual Art Side):**
- **Background:** Soft Gray Canvas with Subtle Grid Lines (`#F4F4F6`)
- **Left Column (Personal & Contact Details):**
  - **Contact Person:** **JULIAN THORNE** (Bold uppercase header typography)
  - **Role / Job Title:** `Principal Photographer & Creative Director` (Clean secondary subheader)
  - **Contact Details (Charcoal Pill Badges):**
    - ✉️ Email Badge: `hello@studiovision.com` (Solid charcoal capsule, white text)
    - 📞 Phone Badge: `+123-456-7890` (Solid charcoal capsule, white text)
    - 🌐 Website Badge: `studiovision.com` (Solid charcoal capsule, white text)
  - **Address Line:** `📍 123 Anywhere St., Any City` (Location pin icon + address text line)
- **Right Column (Topic Visual Artwork):**
  - **Visual Illustration:** Detailed charcoal vector line art illustration of a vintage SLR camera body, lens aperture & camera tripod with sparkle stars (`✦` / `✨`)

---

### ASSET 9 - DISCOUNT VOUCHER COUPON
**Layout:** Horizontal Promotional Voucher Card Set (3 High-Impact Design Variations - Canva & Print Ready)  
**Style & Aesthetics:** Sleek Minimalist Photography Theme with Matte Black & Fine Grid Background & Lens Photography  

#### VOUCHER VARIATION 1: PORTRAIT SESSION PROMO (`20% OFF`)
- **Background & Palette:** Matte Charcoal Black Gradient (`#181818` -> `#050505`)
- **Top Left Header:** Brand Logo (**Studio Vision**)
- **Main Headline:** **PORTRAIT SESSION PROMO** (Bold condensed white display typography)
- **Giant Offer Highlight:** **20% OFF PORTRAIT SHOOT** (Giant centered white typography)
- **Promo Code Pill Badge:** `CODE : VISION20` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution product photography of a professional SLR camera lens aperture with golden flash flare rays & bokeh light reflection

#### VOUCHER VARIATION 2: FREE CANVAS PRINT UPGRADE (`FREE PRINT UPGRADE`)
- **Background & Palette:** Deep Studio Gray Solid (`#2B2B2B`)
- **Top Left Header:** Brand Logo (**Studio Vision**)
- **Main Headline:** **FREE CANVAS PRINT UPGRADE** (Bold white uppercase display typography)
- **Giant Offer Highlight:** **FREE 12x18 CANVAS PRINT** (Giant high-contrast white text highlight)
- **Promo Code Pill Badge:** `CODE : CANVASFREE` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution photo of framed canvas prints pair & polaroid photo stack with camera tripod

#### VOUCHER VARIATION 3: WEDDING & EVENT SPECIAL (`$200 OFF`)
- **Background & Palette:** Obsidian Black & Silver Flare Gradient (`#0F0F10` -> `#000000`)
- **Top Left Header:** Brand Logo (**Studio Vision**)
- **Main Headline:** **WEDDING & EVENT SPECIAL** (Bold white header typography)
- **Giant Offer Highlight:** **$200 OFF FULL-DAY PHOTO PACK** (Giant bold white text highlight)
- **Promo Code Pill Badge:** `CODE : WEDDINGPHOTO200` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution photo showcase of professional photography softbox lighting setup, camera body & photo studio backdrop

---

### ASSET 10 - MENU & PRICELIST
**Layout:** A4 / Printable Price List Sheet
**Header:** **Studio Vision – MENU & PRICELIST**
**Products / Services & Specific Pricing:**
- **Full Day Wedding Photography (2 Photographers)** – $450.00
- **Outdoor Pre-Wedding Concept Shoot** – $260.00
- **Studio Personal / Graduation Portrait** – $65.00
- **Commercial E-Commerce Product Photo (per item)** – $12.00
**Footer Note:** All prices include applicable taxes. Contact us for custom bookings!