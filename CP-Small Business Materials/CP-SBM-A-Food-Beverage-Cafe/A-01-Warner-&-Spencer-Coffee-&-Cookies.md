# JUDUL 1 OF 20
## Warner & Spencer – Coffee & Cookies Shop Promotional Suite

**Sub-Topic:** CP-SBM-A-Food-Beverage-Cafe  
**Main Keywords:** warner & spencer, coffee & cookies shop, warner15, promo banner, loyalty card  
**Target Audience:** Customers, Clients, Local Community, Social Media Followers  
**Style:** Professional Canva Small Business Aesthetic Layout  
**Tone:** Engaging, Friendly, Trustworthy  

**Total Assets:** 10 Aset Promosi Lengkap (IG Story x3, IG Post x2, Loyalty Card, Business Card, Discount Coupon, Menu/Pricelist, Logo)  

---

### ASSET 1 - BRAND LOGO & IDENTITY
**Logo Concept:**
- Logo Name: **Warner & Spencer**
- Tagline: *"Exclusive Sips for Loyal Hearts"*
- Primary Colors: Warm Earthy Bronze, Caramel & Cream
- Visual Style: Clean, modern, high-readability typography suitable for Canva social media banners and print materials.

---

### ASSET 2 - IG STORY 1: "WE'RE OPEN TODAY" (OPERATIONAL ANNOUNCEMENT)
**Format:** Vertical Story (1080x1920) | **Style:** Fast, Fresh, Casual Tone
**Text Content:**
- Header Tag: WE'RE OPEN TODAY! 📍
- Highlight: Ready to serve you the best Salted Caramel Macchiato & Freshly Baked Cookies.
- Operating Hours: 07.00 AM — 09.00 PM (Monday - Sunday)
- Location Pin: 123 Anywhere St., Any City, ST 12345
- Interactive Sticker: Tap link below to get instant directions / WhatsApp chat!

---

### ASSET 3 - IG STORY 2: FLASH SALE & SPECIAL PROMO
**Format:** Vertical Story (1080x1920) | **Style:** High Urgency, Eye-Catching Deal
**Text Content:**
- Big Headline: SPECIAL PROMO — 15% OFF
- Subtext: Exclusive offer for our signature Salted Caramel Macchiato & Freshly Baked Cookies!
- Coupon Code: Use code **WARNER15** at checkout.
- Expiry Notice: ⏰ Valid until September 02, 2035 (Limited Slots)
- Call to Action: Swipe Up / Tap link in bio to claim your promo code now!

---

### ASSET 4 - IG STORY 3: "THANKS FOR COMING TO OUR SHOP" (CUSTOMER APPRECIATION)
**Format:** Vertical Story (1080x1920) | **Style:** Soft Warm Aesthetic, Rounded Photo Frame & Heart Badge
**Text Content:**
- Top Headline: THANKS FOR COMING TO OUR SHOP
- Visual Element: Rounded photo frame of happy customer / staff with heart sticker badge
- Brand Name: **Warner & Spencer**
- Closing Message: "We can't wait to bake something special for you again. See you soon!"

---

### ASSET 5 - IG POST 1: PRODUCT FEATURE SPOTLIGHT BANNER
**Format:** Square Feed (1080x1080) | **Style:** Aesthetic Product Showcase Feed Ad
**Text Content:**
- Logo Header: **Warner & Spencer**
- Big Headline: THE ULTIMATE CARAMEL MACCHIATO
- Subtitle: RICHNESS IN EVERY SIP
- Feature Badge: 100% ARABICA BEANS & HOUSE-MADE CARAMEL
- Visual Element: Premium iced salted caramel macchiato glass with golden caramel drizzle artwork on warm bronze aesthetic layout
- Caption Copy: "Elevate your coffee break with our signature Salted Caramel Macchiato. Rich espresso, velvety milk, and handcrafted caramel in perfect balance."
- CTA Button: Sip The Distinction / Order Now
- Footer Contact: www.reallygreatsite.com | +123-456-7890

---

### ASSET 6 - IG POST 2: FEATURED SELECTION & ITEM SHOWCASE
**Format:** Square Feed (1080x1080) | **Style:** Detailed Product/Service Showcase Card
**Text Content:**
- Title: OUR FEATURED SELECTIONS – Warner & Spencer
- Highlighted Menu/Service Showcase:
  - **Artisan Triple Chocolate Cookie** ($3.25) – Freshly baked daily using 70% Belgian dark chocolate chunks and real French butter.
- Subtext: Crafted with precision, passion, and premium quality.
- CTA: Tap link in bio to view full menu and price list!

---

### ASSET 7 - PROMOTION LOYALTY CARD
**Layout:** Standard Wallet Stamp Card (Front & Back)
**Front Side:**
- Title: LOYALTY CARD – Warner & Spencer
- Reward Rule: Collect 5 stamps to get 1 Free Signature Caramel Macchiato
- Stamp Grid: 5 Stamp Slots (Slot 5 = FREE REWARD)
**Back Side:**
- Logo: **Warner & Spencer**
- Tagline: *"Exclusive Sips for Loyal Hearts"*
- Terms: Non-transferable. Present card upon purchase to collect stamps.

---

### ASSET 8 - BRAND BUSINESS CARD
**Layout:** Standard Business Card Size (Front & Back - Canva Professional Layout)  
**Style & Aesthetics:** Modern Aesthetic Layout with Dark/Solid Olive Green Cover Front & Light Cream Back  

**Front Side (Brand Cover Side):**
- **Background:** Solid Olive Green (`#556B2F`)
- **Center Logo & Illustration:** Off-white minimalist line art vector logo of a matcha/coffee cup bowl & straw whisk artwork
- **Brand Name:** **WARNER & SPENCER** (Bold organic display typography, centered)
- **Tagline:** *"Exclusive Sips for Loyal Hearts"* (Centered subtitle in cream text, in quotes)

**Back Side (Contact Details & Visual Art Side):**
- **Background:** Soft Cream / Light Grid Canvas (`#F5F5DC`)
- **Left Column (Personal & Contact Details):**
  - **Contact Person:** **CHIDI EZE** (Bold uppercase header typography)
  - **Role / Job Title:** `Marketing Manager` (Clean secondary subheader)
  - **Contact Details (Green Pill Badges):**
    - ✉️ Email Badge: `hello@reallygreatsite.com` (Solid green capsule, white text)
    - 📞 Phone Badge: `+123-456-7890` (Solid green capsule, white text)
    - 🌐 Website Badge: `reallygreatsite.com` (Solid green capsule, white text)
  - **Address Line:** `📍 123 Anywhere St., Any City` (Location icon + address line text)
- **Right Column (Topic Visual Artwork):**
  - **Visual Illustration:** Detailed olive green vector line art illustration of a matcha bowl and bamboo whisk (chasen) with decorative sparkle stars (`✦` / `✨`)

---

### ASSET 9 - DISCOUNT VOUCHER COUPON
**Layout:** Horizontal Promotional Voucher Card Set (3 High-Impact Design Variations - Canva & Print Ready)  
**Style & Aesthetics:** Dynamic Coffeehouse Theme with Warm Earthy Bronze Background & High-Definition Food Photography  

#### VOUCHER VARIATION 1: SPECIAL PERCENTAGE DISCOUNT (`30% OFF`)
- **Background & Palette:** Warm Earthy Bronze & Caramel Gradient (`#4A2511` -> `#2C1408`)
- **Top Left Header:** Brand Logo (**Warner & Spencer**)
- **Main Headline:** **DISCOUNT COUPON** (Bold condensed white display typography)
- **Giant Offer Highlight:** **30% OFF** (Giant centered white typography)
- **Promo Code Pill Badge:** `CODE : WARNER30` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution product photography of signature iced caramel macchiato glass with golden caramel drizzle & floating roasted coffee beans

#### VOUCHER VARIATION 2: B1G1 SPECIAL DEAL (`BUY 1 GET 1`)
- **Background & Palette:** Rich Warm Caramel Solid (`#6A381F`)
- **Top Left Header:** Brand Logo (**Warner & Spencer**)
- **Main Headline:** **BUY 1 GET 1** (Bold white uppercase display typography)
- **Giant Offer Highlight:** **BUY 1 GET 1** (Giant high-contrast white text highlight)
- **Promo Code Pill Badge:** `CODE : B1G1` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution photography of freshly baked chocolate chip cookies stacked with melted chocolate chunks & flying cookie crumbs

#### VOUCHER VARIATION 3: PARTY BUNDLE DEAL (`40% OFF`)
- **Background & Palette:** Deep Espresso & Roasted Coffee Gradient (`#36190D` -> `#1A0B05`)
- **Top Left Header:** Brand Logo (**Warner & Spencer**)
- **Main Headline:** **COFFEE & COOKIE BUNDLE** (Bold white header typography)
- **Giant Offer Highlight:** **40% OFF** (Giant bold white text highlight)
- **Promo Code Pill Badge:** `CODE : BUNDLE40` (White pill badge capsule with bold dark text)
- **Expiry Notice:** `Valid until: Dec 30, 2025` (Clean white subtext)
- **Right Visual Artwork:** High-resolution photo showcase of coffee party bucket & assorted cookie box with floating coffee powder dust

---

### ASSET 10 - MENU & PRICELIST
**Layout:** A4 / Printable Price List Sheet
**Header:** **Warner & Spencer – MENU & PRICELIST**
**Products / Services & Specific Pricing:**
- **Iced Salted Caramel Macchiato** – $4.75
- **Artisan Triple Chocolate Cookie** – $3.25
- **Signature Cold Brew Vanilla** – $4.25
- **Fresh Butter Croissant** – $3.80
**Footer Note:** All prices include applicable taxes. Contact us for custom bookings!