# JUDUL 2 OF 5
## How Employee Wellbeing Drives Business Growth Presentation

**Sub-Topic:** A - Reporting & Compliance  
**Main Keywords:** employee wellbeing, workplace wellness, employee wellbeing business growth, workforce health report, people and performance slides, employee engagement report, healthy workplace strategy  
**Audience:** Investors, C-Suite executives  
**Style:** Professional, Corporate, Modern, Clean  
**Tone:** Data-driven, humanistic, growth-focused

**Total Pages:** 10 halaman  
**Pattern Kolom:** 1→1→2→1→4→3→2→1→2→1

---

### PAGE 1 - COVER
**Layout:** Cover page  
**Visual Note:** High-impact professional design using clean minimalist typography. Warm ivory background with sage green accents and a minimalist line-art of an upward growth chart intersecting with a human silhouette. Use Title Case for the title.

**Text:**
- Title: How Employee Wellbeing Drives Business Growth
- Subtitle: A Data-Backed Business Report on the Direct Link Between Workforce Health and Company Performance

---

### PAGE 2 - OPENER
**Fase:** OPENING  
**Tipe Halaman:** Opener  
**Judul Halaman:** Official Report Overview  
**Jumlah Kolom:** 1  
| Kolom 1 |
|---------|
| **Organization:** Liceria & Co.<br>**Report Period:** Fiscal Year 2024<br>**Wellbeing Focus:** Employee Wellbeing as a Driver of Business Growth<br>**Report Lead:** Adeline Palmerston, Head of People & Growth Strategy<br>**Prepared At:** Studio Shodwe<br><br>**Our Aim:** To give investors and C-Suite stakeholders a clear, data-backed case for why employee wellbeing is not a cost — it is a growth driver. This report shows how Liceria & Co. has turned workforce health into measurable business performance. |

**Visual Note:** Clean professional header with high-end sage accents and structured layout. Maintain 40% white space for editability.

---

### PAGE 3 - CONTEXT
**Fase:** OPENING  
**Tipe Halaman:** List/Value  
**Judul Halaman:** Two Pillars Where Wellbeing Drives Growth  
**Jumlah Kolom:** 2  
| Kolom 1 | Kolom 2 |
|---------|---------|
| **People Performance:**<br>Healthy employees produce more, stay focused longer, and deliver higher quality output. Teams that feel supported show lower absenteeism and stronger engagement across every function. | **Business Strength:**<br>Wellbeing directly reduces turnover costs and attracts top talent — while building a workplace reputation that investors, clients, and future hires consistently choose over the competition. |

**Visual Note:** Two bold pillars in sage green and slate. Clean, data-ready design.

---

### PAGE 4 - CORE CONTENT A
**Fase:** CORE CONTENT  
**Tipe Halaman:** Emotional/Reflective  
**Judul Halaman:** The Business Case For Caring About People  
**Jumlah Kolom:** 1  
| Kolom 1 |
|---------|
| At Liceria & Co., we treat employee wellbeing as a core business strategy. Data from Studio Shodwe is clear: teams with higher wellbeing scores outperform on productivity, show lower absenteeism, and stay longer. For investors, that means lower costs, stronger output, and a more stable workforce. |

**Visual Note:** High-impact reflective design with clean serif typography in ivory and slate.

---

### PAGE 5 - CORE CONTENT B
**Fase:** CORE CONTENT  
**Tipe Halaman:** List/Value  
**Judul Halaman:** Our People Performance Numbers  
**Jumlah Kolom:** 4  
| Kolom 1 | Kolom 2 | Kolom 3 | Kolom 4 |
|---------|---------|---------|---------|
| **Wellbeing**<br>Score index. | **Retention**<br>Rate increase. | **Output**<br>Productivity gain. | **Saving**<br>Turnover cost cut. |

**Visual Note:** Data-card design in sage and slate with bold metric labels. Clear, investor-ready layout.

---

### PAGE 6 - CORE CONTENT C
**Fase:** CORE CONTENT  
**Tipe Halaman:** Profile/Spotlight  
**Judul Halaman:** Leadership On People And Performance  
**Jumlah Kolom:** 3  
| Kolom 1 | Kolom 2 | Kolom 3 |
|---------|---------|---------|
| **The People Lead:**<br>Adeline Palmerston, whose wellbeing strategy at Liceria & Co. has delivered measurable gains in team output, retention rates, and overall employee satisfaction scores. | **The Growth Voice:**<br>Drew Feig, whose financial modeling at Liceria & Co. has quantified the direct ROI of people investment and strengthened our growth story for investors. | **The Data Voice:**<br>Jamie Chastain, whose people analytics work at Liceria & Co. turns wellbeing survey data into clear, actionable insights that guide our workforce health decisions. |

**Visual Note:** Trio-profile layout using professional serif typography and slate accents. Use "In-Use" text.

---

### PAGE 7 - ENGAGEMENT BREAK
**Fase:** ENGAGEMENT  
**Tipe Halaman:** Profile/Spotlight  
**Judul Halaman:** Investor Engagement Highlights  
**Jumlah Kolom:** 2  
| Kolom 1 | Kolom 2 |
|---------|---------|
| **The Analyst:**<br>Olivia W. on talent strategy. | **The Advisor:**<br>Alfredo T. on workforce data. |

**Visual Note:** Engagement card layout with a professional, data-forward design. Use "In-Use" text.

---

### PAGE 8 - EMOTIONAL PEAK
**Fase:** PEAK  
**Tipe Halaman:** Emotional/Reflective  
**Judul Halaman:** When People Thrive, Business Follows  
**Jumlah Kolom:** 1  
| Kolom 1 |
|---------|
| Every number in this report started with a person at Liceria & Co. who chose to give their best. When we invested in that person's wellbeing at Studio Shodwe, the business grew — not from a program, but from a real commitment to the people who power our work. |

**Visual Note:** High-impact affirmation design with clean minimalist typography and a subtle human-growth visual accent.

---

### PAGE 9 - CLOSING
**Fase:** CLOSE  
**Tipe Halaman:** Emotional/Reflective  
**Judul Halaman:** Our Commitment To Growth Through People  
**Jumlah Kolom:** 2  
| Kolom 1 | Kolom 2 |
|---------|---------|
| As Liceria & Co. continues to scale, our people strategy at Studio Shodwe remains the foundation of our growth model. Every wellbeing initiative we run is tied to a business outcome. Every outcome we achieve is transparently reported. We invite our investors to see our people not as overhead, but as our most valuable growth asset. | **What Comes Next:**<br>Full workforce performance data, wellbeing program details, and third-party measurement reports are available upon request. To connect with our People & Growth team, contact Adeline Palmerston at Liceria & Co. |

**Visual Note:** Split closing layout — left for strategic commitment, right for actionable next steps. Clean sage and slate finish. Use "In-Use" text.

---

### PAGE 10 - BACK COVER
**Layout:** Back cover  
**Visual Note:** Corporate growth branding with a minimalist sage and slate finish.

**Text:**
- How Employee Wellbeing Drives Business Growth
- Liceria & Co. • Fiscal Year 2024
- www.reallygreatsite.com

---

## NOTES FOR DESIGNER

### Design Philosophy:
- **Tone:** Professional, humanistic, growth-focused investor-grade.
- **Color Palette:** Sage green, slate gray, ivory, soft gold.

### Technical Constraints (Canva Safety):
- **No Effects:** Strictly avoid Shadow, Lift, and Neon text effects for better print quality.
- **60% Rule:** Maintain 40% white space within text boxes for better editability.

### Customization Points:
- Adeline Palmerston (Standardized Name)
- Drew Feig (Standardized Name)
- Jamie Chastain (Standardized Name)
- Alfredo Torres (Standardized Name)
- Olivia Wilson (Standardized Name)
- Liceria & Co. (Generic Business Name)
- Studio Shodwe (Generic Business Name — used as HQ/venue reference only)
- www.reallygreatsite.com (Standard Website)
