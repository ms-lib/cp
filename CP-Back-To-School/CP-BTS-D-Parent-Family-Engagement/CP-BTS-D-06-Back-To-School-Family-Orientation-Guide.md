# JUDUL 6 OF 6
## ___ ___ Back To School Family Orientation Guide Presentation

**Sub-Topic:** D - Parent & Family Engagement  
**Main Keywords:** family orientation guide, back to school presentation, welcome families slides, parent orientation handbook, school year welcome, home school partnership, student activity guide  
**Audience:** Parents  
**Style:** Vibrant, Bright, Structured  
**Tone:** Welcoming, comprehensive, clear  

**Total Pages:** 10 halaman  
**Pattern Kolom:** 1→1→3→1→2→4→2→1→3→1  

---

### PAGE 1 - COVER
**Layout:** Cover page  
**Visual Note:** High-energy bold cover. Vibrant blue background with solid yellow geometric layout boxes and icons representing family shapes, handshakes, and stars. Playful title case typography. Use Title Case for the title.  

**Text:**
- Title: Back To School Family Orientation Guide
- Subtitle: A Comprehensive Handbook for Navigating School Resources, Schedules, and Partnerships

---

### PAGE 2 - OPENER
**Fase:** OPENING  
**Tipe Halaman:** Opener  
**Judul Halaman:** Welcome Families to Orientation  
**Jumlah Kolom:** 1  

| Kolom 1 |
|---------|
| **School:** Studio Shodwe School  <br>**Date:** September 02, 2030  <br>**Facilitator:** Adeline Palmerston  <br><br>Welcome families and guardians! We are absolutely thrilled to welcome you to our family orientation. This guide outlines school logistics, safety policies, parent volunteer systems, and key milestones, helping you prepare your child for a smooth and happy school transition. |

**Visual Note:** High-quality photo placeholder showing a group of smiling elementary school kids in a colorful classroom. Maintain 40% white space for easy editing.  

---

### PAGE 3 - CONTEXT
**Fase:** OPENING  
**Tipe Halaman:** List/Value  
**Judul Halaman:** Three Key Focus Areas  
**Jumlah Kolom:** 3  

| Kolom 1 | Kolom 2 | Kolom 3 |
|---------|---------|---------|
| **School Safety:**<br>Review visitor procedures, parking details, and entry policies. | **Active Contact:**<br>Checking email newsletters, updates, and family portal logins. | **Parent Helper:**<br>Registering for volunteer opportunities and school events. |

**Visual Note:** Three clean columns in bright primary colors with bold circular icons for a key, an envelope, and a handshake.  

---

### PAGE 4 - CORE CONTENT A
**Fase:** CORE CONTENT  
**Tipe Halaman:** Emotional/Reflective  
**Judul Halaman:** School Health and Safety  
**Jumlah Kolom:** 1  

| Kolom 1 |
|---------|
| We prioritize the safety and well-being of every student in our care. By coordinating with school security, practicing drop-off safety, and maintaining clean, supervised classrooms, we provide a secure, comfortable environment where children can thrive. |

**Visual Note:** Center-aligned quote on a soft sky blue background, framed by a border of smiling stars.  

---

### PAGE 5 - CORE CONTENT B
**Fase:** CORE CONTENT  
**Tipe Halaman:** List/Value  
**Judul Halaman:** Drop-Off and Pick-Up  
**Jumlah Kolom:** 2  

| Kolom 1 | Kolom 2 |
|---------|---------|
| **Morning Drop-off:**<br>Arrive between 7:45 and 8:10 AM at the south gate. Helper teachers will guide students directly to homerooms. | **Afternoon Pick-up:**<br>Gather at the south courtyard at 3:00 PM. Present your family ID card to class teachers before signing off. |

**Visual Note:** Two distinct text blocks with thin grey borders on a clean blue background.  

---

### PAGE 6 - CORE CONTENT C
**Fase:** CORE CONTENT  
**Tipe Halaman:** List/Value  
**Judul Halaman:** Essential Checklists  
**Jumlah Kolom:** 4  

| Kolom 1 | Kolom 2 | Kolom 3 | Kolom 4 |
|---------|---------|---------|---------|
| **Health Forms:**<br>Submitting student records. | **Portal Login:**<br>Registering family accounts. | **Supply Bag:**<br>Label all drawing tools. | **Goal Card:**<br>Review family worksheets. |

**Visual Note:** Four bold square cards in primary colors (red, blue, yellow, green) with clean kid-friendly checkmark icons.  

---

### PAGE 7 - ENGAGEMENT BREAK
**Fase:** ENGAGEMENT  
**Tipe Halaman:** Profile/Spotlight  
**Judul Halaman:** Volunteer Sign-Up  
**Jumlah Kolom:** 2  

| Kolom 1 | Kolom 2 |
|---------|---------|
| **Library Helper:**<br>Assist children with finding books, organizing logs, and running story events. | **Event Coordinator:**<br>Support teachers with organizing science nights, class showcases, and field trips. |

**Visual Note:** Two friendly activity columns styled as clipboard cards with green checkmarks.  

---

### PAGE 8 - EMOTIONAL PEAK
**Fase:** PEAK  
**Tipe Halaman:** Emotional/Reflective  
**Judul Halaman:** A Strong Community  
**Jumlah Kolom:** 1  

| Kolom 1 |
|---------|
| A strong school is built on the active engagement and support of its families. When we work together to coordinate events, encourage reading, and maintain open communication, we build a beautiful, supportive community that helps every child succeed. |

**Visual Note:** High-contrast text slide on a sun yellow background with huge colorful quotation marks and hand-drawn stars.  

---

### PAGE 9 - CLOSING
**Fase:** CLOSE  
**Tipe Halaman:** List/Value  
**Judul Halaman:** Ready to Begin  
**Jumlah Kolom:** 3  

| Kolom 1 | Kolom 2 | Kolom 3 |
|---------|---------|---------|
| **Our Commitment:**<br>To support your child's growth and safety. | **Your Part:**<br>Stay connected and check newsletters daily. | **Next Step:**<br>Pick up orientation packets at the lobby. |

**Visual Note:** Split layout using soft blue and yellow panels, dividing final actions into three neat columns.  

---

### PAGE 10 - BACK COVER
**Layout:** Back cover  
**Visual Note:** High-energy back cover matching the cover design, with school details and website centered.  

**Text:**
- Back To School Family Orientation Guide
- Studio Shodwe School
- www.reallygreatsite.com

---

## NOTES FOR DESIGNER

### Design Philosophy:
- **Tone:** Welcoming, comprehensive, and clear.
- **Color Palette:** Bright blue, yellow, orange, cream.

### Technical Constraints (Canva Safety):
- **No Effects:** Strictly avoid Shadow, Lift, and Neon text effects for better print quality.
- **60% Rule:** Maintain 40% white space within text boxes for better editability.

### Customization Points:
- Adeline Palmerston (Standardized Name)
- Studio Shodwe School (Generic School Name)
- www.reallygreatsite.com (Standard Website)
